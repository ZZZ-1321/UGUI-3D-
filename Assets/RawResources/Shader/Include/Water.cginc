...
inline void ComputeGrabPassPos(float4 pos, out float4 grabPassPos)
{
#if UNITY_UV_STARTS_AT_TOP
	float scale = -1.0;
#else
...
half4  ExtinctColor (half4 baseColor, half extinctionAmount) 
{
	...
	half3 GerstnerOffset(half2 xzVtx, half steepness, half amp, half freq, half speed, half2 dir)
	{
		half3 offsets;
	...
	half3 GerstnerNormal (half2 xzVtx, half amp, half freq, half speed, half2 dir) 
	{
	...
	void GerstnerSimple(out half3 offs, out half3 nrml,
		half3 vtx, half3 tileableVtx,
		half4 amplitude, half4 frequency, half4 steepness,
		half4 speed, half4 directionAB, half4 directionCD)
	{
#ifdef WATER_VERTEX_DISPLACEMENT_ON
		offs = GerstnerOffset(tileableVtx.xz, steepness.x, amplitude.x, frequency.x, speed.xz, directionAB.xz);
		nrml = GerstnerNormal(tileableVtx.xz + offs.xz, amplitude, frequency, speed, directionAB);
#else
		offs = half3(0, 0, 0);
		nrml = half3(0, 1, 0);
#endif							
