...
////// Emissive:
                ...
                //float node_1055 = 0.0;
                float2 node_7350 = lerp(_FlowMap_var.rgb.rg,i.uv2,(_MinDist + ( i.vertexColor.a * (_MaxDist - _MinDist) )));
                half4 _Texture_var = tex2D(_Texture,TRANSFORM_TEX(node_7350, _Texture));

#if defined (_NO_ALPHA_CHANNEL)
				_Texture_var.a *= _Texture_var.r;
#endif
                float3 emissive = (lerp( _Texture_var.rgb, 1.0, _node_4352 )*_Color_copy.rgb*i.vertexColor.rgb*_ColorMultiply);

                //float3 finalColor = emissive;
                half4 finalRGBA = half4(emissive,saturate(smoothstep( 0.0, _Smoothness, (_Texture_var.a-(1.0 - i.vertexColor.a)) )));
